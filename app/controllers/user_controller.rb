...
#  model   :user
        ...
        User.transaction(user) do
          ...
          if user.save
            if LoginEngine.config(:use_email_notification)
              ...
              flash[:notice] = _("Updated password emailed to %{email}") % 
                               { :email => @user.email }
            else
  ...
  def forgot_password
    ...
    if !LoginEngine.config(:use_email_notification)
      flash[:message] = _("Please contact the system admin at %{email} to reset your password.") % { :email => LoginEngine.config(:admin_email) }
    ...
    if params[:user][:email].empty?
      ...
      flash.now[:warning] = _("We could not find a user with the email address %{email}") %
      ...
      begin
        User.transaction(user) do
          ...
          flash[:notice] = _("Instructions on resetting your password have been emailed to %{email}") %
            {:email => params[:user][:email]}
        end  
        unless user?
          redirect_to :action => 'login'
    ...
    def do_delete_user(user)
      begin
        ...
        if LoginEngine.config(:use_email_notification)
          ...
          flash.now[:notice] = _("The account has been scheduled for deletion. It will be removed in %{delay_delete_days} days.") % {:delay_delete_days => LoginEngine.config(:delayed_delete_days)}
        end
        return false
      end
    end
  ...
  def restore_deleted
    ...
    @user.deleted = 0
    if not @user.save
      flash.now[:warning] = _("The account for %{login} was not restored. Please try the link again.") % {:login => @user['login']}
      redirect_to :action => 'login'
    else
  ...
  def destroy(user)
    ...
    flash[:notice] = _("The account for %{login} was successfully deleted.") % {:login => user['login']}
    user.destroy()
  end
